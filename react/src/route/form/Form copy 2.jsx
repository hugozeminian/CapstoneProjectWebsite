...
const Form = () => {
        ...
        if (field.name === name) {
          ...
          switch (`${key}-${name}`) {
            ...
            case `${key}-${field.name}`:
              ...
              // Example validation: telephone number validation: Must be in the format (XXX) XXX-XXXX
              if (
              ...
              // Example validation: email format and comparing
              if (
                name === "client_email" ||
                ...
                setEmailCompare((prevEmailCompare) => {
                  ...
                  console.log(
                    "🚀 ~ setEmailCompare ~ newEmailCompare:",
                    newEmailCompare
                  );
                ...
                if (
                  ...
                  if (!isValidComparedEmail) {
                    console.log("isValidComparedEmail client");
                    return (error = true);
                  } else {
                    error = false;
                  }
                  console.log(
                    "🚀 ~ initialWeddingDataForm[key].forEach ~ isValidComparedEmail:",
                  ...
                  if (!isValidComparedEmail) {
                    console.log("isValidComparedEmail celebrant");
  ...
  const compareEmails = (emails, userType) => {
    console.log("🚀 ~ compareEmails ~ emails:", emails);
    ...
    console.log(
      "🚀 ~ compareEmails ~ confirmEmail === email:",
      confirmEmail === email
    );
  ...
  const handleChange = (event, formDataKey, item, index) => {
    ...
    // Copy the existing formDataErrorUpdated object
    const updatedFormDataError = { ...formDataErrorUpdated };

    // Retrieve the specific array (client, celebrant, wedding) based on the key
    let formDataArray = updatedFormDataError[formDataKey];

    // Check if formDataArray exists, if not create an empty array
    if (!formDataArray) {
      formDataArray = [];
    }

    // Update the error for the specified index
    formDataArray[index] = {
      ...formDataArray[index],
      error: error,
    };

    // Update the formDataErrorUpdated object with the modified array
    updatedFormDataError[formDataKey] = formDataArray;

    // Update the state with the modified formDataErrorUpdated object
    setFormDataErrorUpdated(updatedFormDataError);
